...
- `Selected Work` entrou após Serviços com `Unemployment Killer`, `CellTech Aracaju` e uma área separada de estudos visuais; os itens mantêm estado explícito até a curadoria final dos projetos e cases.
...
- A passagem da abertura para Serviços usa um handoff editorial próprio: a cena final da hero não corta diretamente para a próxima seção; ela entrega para uma zona sticky curta sustentada por chamada tipográfica grande. O fundo começa com a luz e os tons escuros da hero e se resolve no mesmo preto quente da seção de serviços, conectando os dois capítulos sem uma borda de corte. A referência Raw Cereal contribui com o princípio de intervalo narrativo entre capítulos, mas a execução usa conteúdo, paleta e matéria visual do Portfolio v2. No mobile o handoff é mais direto; com movimento reduzido, a composição fica estática.
- Em `Selected Work`, cada projeto ocupa um trecho sticky próprio: a imagem se revela por varredura horizontal sob uma grade de sinal monocromática durante a primeira metade do trecho e permanece completamente visível antes da troca para o próximo projeto. Título e metadados chegam em tempos diferentes. A referência dominante dessa apresentação é Yannick Gregoire; mobile e movimento reduzido usam o estado final sem sticky.
